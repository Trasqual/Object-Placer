...
namespace TnTStudio.ObjectPlacer
{
    public class ObjectPlacer : EditorWindow
    {
        ...
        {
            if (AssetDatabase.IsValidFolder("Assets/ObjectPlacer/Presets"))
            {
                string[] guids = AssetDatabase.FindAssets("t:ObjectGroup", new[] { "Assets/ObjectPlacer/Presets" });
        ...
        private void AddGroup(string name)
        {
            ...
            if (AssetDatabase.IsValidFolder("Assets/ObjectPlacer/Presets"))
            {
                string path = $"Assets/ObjectPlacer/Presets/{newGroup.groupName}.asset";
                AssetDatabase.CreateAsset(newGroup, path);
            }
            else
            {
                AssetDatabase.CreateFolder("Assets/ObjectPlacer", "Presets");
                string path = $"Assets/ObjectPlacer/Presets/{newGroup.groupName}.asset";
                AssetDatabase.CreateAsset(newGroup, path);
            }
        }
        ...
        private void RemoveGroup(ObjectGroup group)
        {
            ...
            string path = $"Assets/ObjectPlacer/Presets/{group.groupName}.asset";
